...
public class AdminService
{
    ...
    public async Task<AdminDashboardResponse> GetDashboardStatsAsync(CancellationToken cancellationToken = default)
    {
        ...
        var pendingOwnerRegistrations = await _ownerRegistrationRepository.CountPendingAsync(cancellationToken);
        var pendingSubmissions = await _ownerSubmissionRepository.CountPendingAsync(cancellationToken);
        var totalPoiViews = await _analyticsRepository.CountDistinctPageViewsByEventNameAsync(SharedConstants.AnalyticsEvents.PoiViewed, cancellationToken);
        var totalAudioPlays = await _analyticsRepository.CountByEventNamesAsync([SharedConstants.AnalyticsEvents.AudioPlayed, SharedConstants.AnalyticsEvents.TtsPlayed], cancellationToken);
        ...
        return new AdminDashboardResponse
        {
            ...
            PendingOwnerRegistrations = pendingOwnerRegistrations,
            PendingSubmissions = pendingSubmissions,
            TotalPoiViews = totalPoiViews,
    ...
    public async Task ApproveOwnerAsync(string adminUserId, string id, CancellationToken cancellationToken = default)
    {
        ...
        if (registration.Status != SharedConstants.OwnerStatuses.Pending)
        {
        ...
        registration.Status = SharedConstants.OwnerStatuses.Approved;
        registration.AdminNote = null;
        registration.ReviewedAt = DateTime.UtcNow;
        registration.ReviewedBy = adminUserId;
        user.OwnerStatus = SharedConstants.OwnerStatuses.Approved;
    ...
    public async Task RejectOwnerAsync(string adminUserId, string id, RejectOwnerRegistrationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var user = await _userRepository.GetByIdAsync(registration.UserId, cancellationToken)
            ?? throw new ApiException("Không tìm thấy user.", StatusCodes.Status404NotFound);
        registration.Status = SharedConstants.OwnerStatuses.Rejected;
        registration.AdminNote = request.AdminNote;
        registration.ReviewedAt = DateTime.UtcNow;
        registration.ReviewedBy = adminUserId;
        user.OwnerStatus = SharedConstants.OwnerStatuses.Rejected;
        await _ownerRegistrationRepository.UpdateAsync(registration, cancellationToken);
        await _userRepository.UpdateAsync(user, cancellationToken);
    }
    ...
    public async Task DisableOwnerAsync(string adminUserId, string id, CancellationToken cancellationToken = default)
    {
        var registration = await _ownerRegistrationRepository.GetByIdAsync(id, cancellationToken)
            ?? throw new ApiException("Không tìm thấy đăng ký owner.", StatusCodes.Status404NotFound);
        if (registration.Status != SharedConstants.OwnerStatuses.Approved)
        {
            throw new ApiException("Chỉ có thể vô hiệu hóa đối tác đã được duyệt.");
        }

        var user = await _userRepository.GetByIdAsync(registration.UserId, cancellationToken)
            ?? throw new ApiException("Không tìm thấy user.", StatusCodes.Status404NotFound);

        user.OwnerStatus = SharedConstants.OwnerStatuses.None;
    ...
    public async Task ApproveSubmissionAsync(string adminUserId, string id, CancellationToken cancellationToken = default)
    {
        ...
        if (submission.Status != SharedConstants.SubmissionStatuses.Pending)
        {
            throw new ApiException("Chỉ có thể duyệt đề xuất đang chờ duyệt.");
        }

        if (submission.SubmissionType == SharedConstants.SubmissionTypes.Create)
        {
            var poi = new Poi
            {
                ...
                IsActive = true,
                AudioStatus = SharedConstants.AudioStatuses.Pending
            };
            await _poiRepository.CreateAsync(poi, cancellationToken);
            submission.PoiId = poi.Id;
        }
        else if (submission.SubmissionType == SharedConstants.SubmissionTypes.Update)
        {
        ...
        submission.Status = SharedConstants.SubmissionStatuses.Approved;
    ...
    public async Task RejectSubmissionAsync(string adminUserId, string id, RejectSubmissionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        submission.Status = SharedConstants.SubmissionStatuses.Rejected;
